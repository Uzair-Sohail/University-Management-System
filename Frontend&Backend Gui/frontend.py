...
# --------------------------------------FUNCTIONS-------------------------------------------------------------------
        ...
        def addData():
            if(len(StdID.get())!=0):
                dbbackend.db.addStdRec(StdID.get(), Firstname.get(), Surname.get() , DoB.get() ,Age.get(), Gender.get(), Address.get(), Mobile.get())
                studentlist.delete(0, END)
                studentlist.insert(END, (StdID.get(), Firstname.get(), Surname.get(), DoB.get(), Age.get(), Gender.get(), Address.get(), Mobile.get()))

        def DisplayData():
            studentlist.delete(0,END)
            for row in dbbackend.db.viewData(self):
        ...
        def DeleteData():
            if(len(StdID.get())!=0):
                dbbackend.db.deleteRec(sd[0])
                clearData()
                DisplayData()

        def searchDatabase():
            studentlist.delete(0,END)
            for row in dbbackend.db.searchData(StdID.get(), Firstname.get(), Surname.get() , DoB.get() ,Age.get(), Gender.get(), Address.get(), Mobile.get()):
                studentlist.insert(END, row, str(""))

        def update():
            if (len(StdID.get()) != 0):
                dbbackend.db.dataUpdate(StdID.get(),Firstname.get() ,DoB.get(), Gender.get() , Address.get(), Mobile.get());                # dbbackend.db.addStdRec(StdID.get(), Firstname.get(), Surname.get(), DoB.get(), Age.get(), Gender.get(),Address.get(), Mobile.get())
                DisplayData()
                # studentlist.delete(0, END)
                # studentlist.insert(END, (StdID.get(), Firstname.get(), Surname.get(), DoB.get(), Age.get(), Gender.get(), Address.get(), Mobile.get()))
...
#--------------------------------entries-------------------------------------------------------------------------------------------------
        ...
        self.lblAge = Label(DataFrameLEFT, font=('times new roman', 20, 'bold'), text="Phone:", padx=2, pady=2,bg="Ghost White")
